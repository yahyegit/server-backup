 <?php
   ...
   function get_users($id){ 
         ...
         $cust_q = mysqli_query_("select `id`,`username`, `full_name`,`current_remaining_limit` from users where delete_status !='1'   ORDER BY username ASC ");   
     while($aRow = mysqli_fetch_assoc_($cust_q) ){
 
            $selected  = ($id == $aRow['id'])?"selected='selected'":'';

               $options .= "<option $selected  rem=\"{$aRow['current_remaining_limit']}\" user_id=\"{$aRow['id']}\">{$aRow['username']}</option>";  
...
<table class='table' style='    margin-left: 0px;width:auto !important; box-shadow: none !important;'>
 ...
 <tr>
   ...
   <input type='text' class='rem'      value=''> 
  </div></td>
