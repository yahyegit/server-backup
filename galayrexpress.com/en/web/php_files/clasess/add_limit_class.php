          ...
          $data['amount'] = str_replace(',','',$data['amount']);
            ...
            $password = (!empty($data['password']))?",password='".enc_password($data['password'])."'":'';
             if (!empty($data['id'])) {
                      ...
                      // update 
                       mysqli_query_("UPDATE `limit_history` set amount='{$data['amount']}',   user_id='{$data['user_id']}', description='{$data['description']}', `date`='{$data['date']}'  where id={$data['id']}"); 
                 
              }else{
 
                         mysqli_query_("INSERT INTO `limit_history`( `amount`, `user_id`, `description`, `date`) VALUES ('{$data['amount']}','{$data['user_id']}','{$data['description']}','{$data['date']}')");
