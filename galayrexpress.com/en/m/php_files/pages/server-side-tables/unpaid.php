 ...
 <?php
	...
	$aColumns = explode(",",'paid_date,commission,id,status,amount,sender_id_no,pay_to_id,sender_name,sender_mobile,pay_to_name,pay_to_mobile,description,user_id');

	$sIndexColumn =  'id';  
	$other_query = "status='unpaid'";
  ...
  if(trim($other_query) == "''" || trim($other_query) == "undefined''"){
	...
	$output = array(
 	...
 	// 		 <th>  Refrence No </th> <th> Pay to </th>  <th> Amount </th>  <th> Status </th> <th> Sender </th>   <th> Description </th>  <th> Action </th>
          $row = array();
	  $s_id  = (!empty($aRow['sender_id_no']))?"<p>
	<label class=\"show_ float_label\">:</label>{$aRow['sender_id_no']}</p>":'';
...
// delete if
    ...
    margin: 0px !important; margin-top: 8px !important;
