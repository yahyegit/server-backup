...
function cal_date(from,to){
	...
	var d1 = new Date(cleanDate(from));
	var d2 = new Date(cleanDate(to));
		...
		if(status != 'submit'){
			...
			window.location.href='#rent_car_form';
				 	...
				 	if(submit_states !='on'){
							...
							return false;} if(empty(rent_paid)){
								error_func('Rent paid is empty');
								$('#rent_paid').css('border','1px solid red');
							return false;}
								
								var d1 = new Date(cleanDate(rent_from_date));
								var d2 = new Date(cleanDate(rent_to_date));
								if(d1.getTime() > d2.getTime()){
