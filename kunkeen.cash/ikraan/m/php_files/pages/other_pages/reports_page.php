...
\" > Report date   <input type='date' id='date_from'  value=\"$full_date_DB\"  onchange=\"chosen_report_date('reports_page.php');\" >  
 ...
 <table  class='table  ' style='width:90%'  ><thead>   <th>   </th>   </tr></thead>
    <tbody>
...
\">  <ul style=\"
 \" >  
  ...
  <p class='title_'>   sales for <span> $full_date </span>  </p> 
    ...
    if_logged_in('die');
      if (is_admin($current_user)) {
              ...
              $date_to =  $_POST['data']['date']['date_to'];
                ...
                if_logged_in('die');
                 $date_full = (strtolower(trim($date_from)) == 'latest' || strtolower(trim($date_from)) == '')?mysqli_result_(mysqli_query_("SELECT `date` from customer_items  where delete_status!='1' ORDER BY `date` DESC LIMIT 1"),0):sanitize(strtolower($date_from));
