...
\" >   date  <input type='date' id='date_from'  user_id='{$user['id']}' value=\"$full_date_DB\"  onchange=\"chosen_report_date('user_account.php');\" >  
 ...
 <input  type='date' id='date_to'  value=\"$date_to\" style=\"display: ".(($date_to !='')?'':'none').";  \" onchange=\" if($.trim($('#date_from').val()) != ''){chosen_report_date('user_account.php');}\">  </p>
  ...
  <div class='it_'>   
         <p class='title_'>   Expenses by <b> $username </b> on <b>$full_date</b> </p> 
    ...
    <p class='title_'>   sales by <b> $username </b> on <b>$full_date</b>  </p> 
    <p><label class='show_ float_label' > Total price  : </label>$ccc".number_format(mysqli_result_(mysqli_query_("select sum(price) from customer_items where user_id='{$user['id']}' and $date_qry and delete_status!='1' "),0),2)." </p>
    ...
    <tbody>
     ...
     <p class='title_'>   payments by <b> $username </b> on <b>$full_date</b> </p> 

     <div style='text-align:center'>
      <p><label class='show_ float_label' > Total payments on: </label>$ccc".number_format(mysqli_result_(mysqli_query_("select sum(amount) from payments where user_id='{$user['id']}' and $date_qry and  delete_status!='1' "),0),2)." </p>
