...
// connect to the MySQL server
    ...
    $company_name_db = str_replace(' ','',strtolower("inventory_".$company_name));
...
if ($connection->query($sql) === TRUE) {
        ...
        // import the sql file  
				...
				$query = '';
				$sqlScript = file('inventory_db.sql');
				foreach ($sqlScript as $line){
					...
					if ($endWith == ';') {
						...
						mysqli_query($connection,"update settings set current_lang='$current_lang',password='$password',username='$username',company_name='$company_name', pw_last_changed='".date('Y-m-d')."' ");	

						mysqli_query($connection,"update users set password='$password',username='$username' where type='admin' limit 1");
							mysqli_query($connection,"update settings set company_name='$company_name', currency='$currency' ");
					}
				}
