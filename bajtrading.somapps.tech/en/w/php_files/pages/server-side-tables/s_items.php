...
$sWhere_query = " `order_number` LIKE '%".sanitize($_GET['sSearch'])."%'  OR  `date` LIKE '%".sanitize($_GET['sSearch'])."%'    ";  
...
$prev_order = array();
    ...
    // all queries is static 
      while($aRow = mysqli_fetch_assoc_($rResult)){
        ...
        if(in_array($aRow['order_number'],$prev_order)){
          continue;
        }else{
            $prev_order[] = $aRow['order_number'];
        }
...
$oDesc = mysqli_result_(mysqli_query_("select description from recieved_history where order_number='{$aRow['order_number']}'  order by id asc limit 1"),0);
 
$oDesc = (!empty($oDesc))?" <td> <span class='gray' style='margin-left:20px ' > Description </span> $oDesc </td>":'';
...
$ot .=  "  <table class='table_order  ' >   <tbody> ";

 $ot .=  "<tr   > <td  >      <span class='gray' style='margin-left: '   >  Total cost </span>  $ccc".number_format($total_price,2)."   <span class='gray' style='margin-left:20px ' > date </span>  ".date('d-M-Y',strtotime($o_date))."     </td> <td>   <span class='gray' style='margin-left:20px ' > Order No </span> {$aRow['order_number']}</td> $oDesc    
