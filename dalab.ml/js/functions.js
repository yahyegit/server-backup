...
function calc_balance(){
		price = Number($('.sell_form .price').val().replace(/,/g , ''));
		paid = Number($('.sell_form .paid').val().replace(/,/g , ''));
		quantity = Number($('.sell_form .quantity').val().replace(/,/g , ''));
		quantity = (!quantity)?1:quantity;

        $('.sell_form .balance').val(CommaFormattedN((price * quantity) - paid));
         if($('.sell_form .balance').val().includes('-')){
         	 $('.sell_form .balance').css('border','2px solid red');
 				error_func('invalid balance');
          }else{
           $('.sell_form .balance').css('border','---');
 				   $('#error').hide();
          }
}
...
function chosen_report_date(){
	...
	to_date = $("input#date_to").val();
	 ...
	 get_template.reports({date_to:to_date,date_from:from_date},'pages/other_pages/expenses_page.php');
...
function error_func(msg){
	...
	if($('form').is(':visible')){
			$('div#error').css('width','50%').css('margin','auto');
	}else{
			$('div#error').css('width','auto').css('margin','');
	}

  	$('div#error').html('<img src="css/error.jpg" alt="error" style="border-radius:2em; width:60px; height:40px; margin-right: 4px;"/>'+msg+"<a href='#' onclick=\"close_element('div#error');\" class='ui-button ui-widget ui-state-default ui-corner-all ui-button-icon-only ui-dialog-titlebar-close' role='button' aria-disabled='false' title='close' style='margin-left: 10px;'><span class='ui-button-icon-primary ui-icon ui-icon-closethick'>close</span><span class='ui-button-text'>close</span></a>").fadeIn(); 
  	 $('#error').delay('60000').fadeOut();
}
...
if($.trim($(rate_input_id).attr('defaut_done')) != 'true'){
  $(rate_input_id).show().val($(rate_input_id).attr('value')).attr('defaut_done','true');		
 	...
 	selected_rate = Number($(rate_input_id).val().replace(/,/g , ''));
...
function new_current_balance(){
	...
	if(trans_type == 'in' || trans_type == 'out'){
		if(Number($('.balance_converted_to_dollar').attr('balance')) && Number($('.balance_converted_to_ksh').attr('balance'))){
				// both conveted 
	                conv = '<br><p style=" margin-top: 4px;    margin-left: 72px;"> converted from <span style="display:none !important" _></span>'+CommaFormattedN($("#amount_money_ksh").val().replace(/,/g , ''))+'<span _ style="display:none !important" ></span> ksh , sell rate: '+CommaFormattedN($("input#rate_dollar").val())+' ksh <br>'; 
					conv +=  '  and $<span style="display:none !important" _></span>'+CommaFormattedN($("#amount_money_dollar").val().replace(/,/g , ''))+'<span _ style="display:none !important"></span>, puy rate :'+CommaFormattedN($("input#rate_ksh").val())+'ksh </p>';  

 		}else{ // single conv//   
				if(Number($('.balance_converted_to_dollar').attr('balance'))){
	                conv = '<br><p style=" margin-top: 4px;    margin-left: 72px;"> converted from <span style="display:none !important" _></span>'+CommaFormattedN($("#amount_money_ksh").val().replace(/,/g , ''))+'<span _ style="display:none !important" ></span> ksh, sell rate: '+CommaFormattedN($("input#rate_dollar").val())+' ksh </p>'; 
				}else if(Number($('.balance_converted_to_ksh').attr('balance'))){
					conv =  '<br><p style=" margin-top: 4px;    margin-left: 72px;"> converted from $<span style="display:none !important" _></span>'+CommaFormattedN($("#amount_money_dollar").val().replace(/,/g , ''))+'<span _ style="display:none !important" ></span>, puy rate was:'+CommaFormattedN($("input#rate_ksh").val())+' ksh </p>';  
				}
		...
		if(Number($('.balance_converted_to_dollar').attr('balance')) && Number($('.balance_converted_to_ksh').attr('balance'))){
				// both conveted 
					conv = '<b><b class="">Exchange:</b>  $'+CommaFormattedN($("#amount_money_dollar").val().replace(/,/g , ''))+'  <span><i>To</i></span> '+CommaFormattedN(Number($('.balance_converted_to_ksh').attr('balance')))+' ksh,  <span>puy rate:  '+CommaFormattedN($("input#rate_ksh").val())+' ksh </span> ';
					conv += '</b> <br><p tyle=" margin-top: 4px;    margin-left: 72px;">  and  <b>'+CommaFormattedN($("#amount_money_ksh").val().replace(/,/g , ''))+' ksh</b> <span><i>To</i></span> <b>$'+CommaFormattedN(Number($('.balance_converted_to_dollar').attr('balance')))+'</b>,  <span> sell rate: '+CommaFormattedN($("input#rate_dollar").val())+' ksh  </span> </p>';
		}else{ // single conv 
				if(Number($('.balance_converted_to_dollar').attr('balance'))){
					conv = '<b><b class=" ">Exchange:</b>  '+CommaFormattedN($("#amount_money_ksh").val().replace(/,/g , ''))+' ksh <span><i>To</i></span> $'+CommaFormattedN(Number($('.balance_converted_to_dollar').attr('balance')))+',  <span> sell rate:  '+CommaFormattedN($("input#rate_dollar").val())+' ksh  </span>  </b>';
				}else if(Number($('.balance_converted_to_ksh').attr('balance'))){
					conv = '<b><b class=" ">Exchange:</b>  $'+CommaFormattedN($("#amount_money_dollar").val().replace(/,/g , ''))+'  <span><i>To</i></span> '+CommaFormattedN(Number($('.balance_converted_to_ksh').attr('balance')))+' ksh, <span>  puy rate: '+CommaFormattedN($("input#rate_ksh").val())+' ksh </span>   </b>';
				}
	...
	if(trans_type == 'exchange'){
				 ...
				 if(dollar_x_amount != '0' && ksh_x_amount != '0'){ // both 
			 	 ...
			 	 msg_type = ' <b> <b class="title_">'+trans_type+':</b> '+value+'  </b>'+conv+'<p style="float: right;color: black;"><span> Balance : </span><span class="'+(( JSON.stringify(new_bl_c).includes('-'))?'debt_color':'in_color')+'" style="font-weight:normal">'+CommaFormattedN(new_bl_c)+' ksh </span><span> and </span><span class="'+((JSON.stringify(new_bl_d).includes('-'))?'debt_color':'in_color')+'" style="font-weight:normal">$'+CommaFormattedN(new_bl_d)+'</span> </p>';			
		} 
...
function delete_(data){
	$('#warning').html('<form style="display:none;" id="delete_form"><input type="hidden" name="id" value="'+data.id+'">  <input type="hidden" name="table" value="'+data.table+'">  </form> '+'<img src="css/warning.png" alt="warning" style="border-radius:2.5em; width:80px; height:40px; margin-right: 4px; "/>'+data.msg).dialog({  show: "blind", hide: "explode", width: 'auto',  height:'auto',  modal: false, buttons:  {
							"Yes": function() {
