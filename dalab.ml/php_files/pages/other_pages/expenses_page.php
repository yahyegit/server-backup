...
<h3 class='title_'>(<strong>".number_format(mysqli_result_(mysqli_query_("select count(id) from products where  ".((trim($date_to) != '')?" date BETWEEN '$full_date_DB' AND '".sanitize($date_to)."' ":" date like '%$full_date_DB%' ")),0))."</strong>) expenses (<i>$full_date</i>) </h3> 
</div>
  
<table class='table dataTable'  table_file='php_files/pages/server-side-tables/expenses.php'  other_query=\"$other_query_\" >
 <thead><tr><th> name </th> <th>Quantity</th><th>cost</th><th>Description</th> <th>date</th> <th>Action</th>  </tr></thead> 
  <tbody>
...
if(isset($_POST['data'])){
  ...
  $date_to =  $_POST['data']['date']['date_to'];
    ...
    if_logged_in('die');
     $date_full = (strtolower(trim($date_from)) == 'latest' || strtolower(trim($date_from)) == '')?mysqli_result_(mysqli_query_("SELECT `date` from expenses  where delete_status!='1' ORDER BY `date` DESC LIMIT 1"),0):sanitize(strtolower($date_from));
