...
function register_cust($data){
 ...
 $data['customer_name'] = (empty($data['customer_name']))?'No name':$data['customer_name'];
    ...
    if(!empty($id)){
          mysqli_query_("update customers set current_balance='".sanitize($data['balance'])."', email='".sanitize($data['email'])."', last_date='".date('Y-m-d')."', mobile='".sanitize($data['mobile'])."' where id=$id");
         return $id;
    }else{
	...
	if(check_token($data['crf_code'],'check')){
          ...
          $date =   sanitize($data['date']);
           ...
           // instert items 
              ...
              $rem = mysqli_result_(mysqli_query_("select remainings from items where item_name='$item_name'"),0);
            $remaining_cost =   mysqli_result_(mysqli_query_("select remaining_cost from items where item_name='$item_name'"),0);
              ...
              if ($rem < $quantity) {
                ...
                $rem = $rem-$quantity;
               mysqli_query_("UPDATE items SET  remainings=$rem, remaining_cost=remaining_cost-$single_cost   where item_name='$item_name' ");
