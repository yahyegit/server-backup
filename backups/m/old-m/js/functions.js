...
function update_bl_item(){
	$('input[name="remainings"]').val(clean($('input[name="remainings"]').attr('rem'))+clean($('input[name="quantity"]').val()));
}
...
function delete_(data){
	$('#warning').css('color','red !important;').html('<form class="form" style="display:none;" id="delete_form"><input type="hidden" name="id" value="'+data.id+'">  <input type="hidden" name="table" value="'+data.table+'">  </form> '+'<img src="css/warning.png" alt="warning" style="border-radius:2.5em; width:80px; height:40px; margin-right: 4px; "/>'+data.msg).dialog({  show: "blind", hide: "explode", width: 'auto',  height:'auto',  modal: true, buttons:  {
							"Yes": function() {
