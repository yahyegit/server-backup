...
//      <th> item  </th> <th>  quantity </th><th> price </th>  <th> balance </th>  <th> action </th> 
  ...
  $ot =  "    <table class='table_order' style='width:auto'  >  <thead> <tr> <th>  item </th> <th>  quantity </th> <th> price </th>   </tr>  </thead>   <tbody> ";
  $total_price = 0;
  $total_balance = 0;
...
$username = "<button class='  ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary'  title='click to see full user account'  onclick=\"request_template('',{id:{$aRow['user_id']}},'pages/other_pages/user_account.php');\"  role='button' aria-disabled='false' style='  display:inline;'><span class='ui-button-icon-primary ui-icon ui-icon- '></span><span class='ui-button-text'>$username  </span></button>"; 
 	...
 	$delevered_by = (!empty($aRow['delevered_by']))?"<span class='gray' style='margin-left:".((trim($aRow['address']) !='')?"20px;":''  )."' > delivered by </span> <button class='  ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary'  title='see all items delivered by {$aRow['delevered_by']} '   onclick=\"request_template('',{delevered_by:'{$aRow['delevered_by']}'},'pages/other_pages/delever_account.php');\"   role='button' aria-disabled='false' style='  display:inline;'><span class='ui-button-icon-primary ui-icon ui-icon- '></span><span class='ui-button-text'>{$aRow['delevered_by']}   </span></button>":'';


   $address = (trim($aRow['address']) !='')?" <span class='gray' style='margin-left: ' > address</span> {$aRow['address']} ":'';
$hide = (trim($aRow['address']) !='' && !empty($aRow['delevered_by']))?"":"style='display:none;'";

$hide_ = (trim($customer_name) !='' && !preg_match('/customer_id/',$other_query))?"":"style='display:none;'";
	$ot .=  "</tbody></table> <table class='table_order  ' style='width:auto'>   <tbody> ";

     $ot .=  "<tr> <td>  <span class='gray' style='margin-left: '   >  total price </span>  $ccc".number_format($total_price,2)."    <span class='gray' style='margin-left:20px ' > Balance </span>  $ccc".number_format($total_balance,2)."   </td>     </tr>




<tr $hide > <td $hide>     $address  $delevered_by   </td> </tr>
<tr> <td>     <span class='gray' style='margin-left: ' > sold by </span>  $username  <button class='  ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary' onclick=\"delete_({table:'$sTable',id:{$aRow['order_number']},msg:'Deleting order from <b> $customer_name </b> ?'}); \" role='button' aria-disabled='false' style='  display:inline;'><span class='ui-button-icon-primary ui-icon ui-icon-trash'></span><span class='ui-button-text'>Delete order  </span></button>       </td>     </tr>
<tr> <td>  <span class='gray'  > Order No </span> {$aRow['order_number']} </td></tr>

<tr  $hide_> <td $hide_ >   <span class='gray' style='margin-left: ' > customer name </span>  

<button  class='change ui-button ui-widget ui-state-default ui-corner-all  ui-button-text-icon-primary' onclick=\"r_page('',{id:{$aRow['customer_id']}},'pages/other_pages/customer_account.php');\" role='button' aria-disabled='false' style='font-size: 11px;'><span class='ui-button-icon-primary ui-icon-   '></span><span class='ui-button-text'>$customer_name</span></button>
