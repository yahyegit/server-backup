...
function chosen_report_date(type){
...
*/
 	...
 	selected_rate = $(rate_input_id).val().replace(/0\./g , '');
...
function new_current_balance(){
	...
	dollar_x_amount = (return_digit($('.balance_converted_to_dollar').attr('balance')) +  return_digit(((Number($('.balance_converted_to_ksh').attr('balance')))?0:$('#amount_money_dollar').val().replace(/,/g , ''))));
		...
		if(trans_type == 'out'){
				x_func(return_digit(current_customer.current_cash_balance) - ksh_x_amount, return_digit(current_customer.current_dollar_balance) - dollar_x_amount); 	 
	...
	if(trans_type == 'in' || trans_type == 'out'){
		if(Number($('.balance_converted_to_dollar').attr('balance')) && Number($('.balance_converted_to_ksh').attr('balance'))){
				// both conveted 
	                conv = '<br><p style=" margin-top: 4px;    margin-left: 72px;"> converted from '+CommaFormattedN($("#amount_money_ksh").val().replace(/,/g , ''))+' ksh , sell rate: '+CommaFormattedN($("input#rate_dollar").val())+' ksh <br>'; 
					conv +=  '  and  $'+CommaFormattedN($("#amount_money_dollar").val().replace(/,/g , ''))+', puy rate :'+CommaFormattedN($("input#rate_ksh").val())+'ksh </p>';  

 		}else{ // single conv//   
				if(Number($('.balance_converted_to_dollar').attr('balance'))){
	                conv = '<br><p style=" margin-top: 4px;    margin-left: 72px;"> converted from '+CommaFormattedN($("#amount_money_ksh").val().replace(/,/g , ''))+' ksh, sell rate: '+CommaFormattedN($("input#rate_dollar").val())+' ksh </p>'; 
				}else if(Number($('.balance_converted_to_ksh').attr('balance'))){
					conv =  '<br><p style=" margin-top: 4px;    margin-left: 72px;"> converted from $'+CommaFormattedN($("#amount_money_dollar").val().replace(/,/g , ''))+', puy rate was:'+CommaFormattedN($("input#rate_ksh").val())+' ksh </p>';  
				}
		...
		if(Number($('.balance_converted_to_dollar').attr('balance')) && Number($('.balance_converted_to_ksh').attr('balance'))){
				// both conveted 
					conv = '<b><b class="title_">Exchange:</b>  $'+CommaFormattedN($("#amount_money_dollar").val().replace(/,/g , ''))+'  <span>To</span> '+CommaFormattedN(Number($('.balance_converted_to_ksh').attr('balance')))+' ksh,  <span>puy rate:</span>  '+CommaFormattedN($("input#rate_ksh").val())+' ksh  ';
					conv += '</b> <br><p style=" margin-top: 4px;    margin-left: 72px;">  and  '+CommaFormattedN($("#amount_money_ksh").val().replace(/,/g , ''))+' ksh <span>To</span> $'+CommaFormattedN(Number($('.balance_converted_to_dollar').attr('balance')))+',  <span> sell rate: </span>'+CommaFormattedN($("input#rate_dollar").val())+' ksh  </p>';
		}else{ // single conv 
				if(Number($('.balance_converted_to_dollar').attr('balance'))){
					conv = '<b><b class="title_">Exchange:</b>  '+CommaFormattedN($("#amount_money_ksh").val().replace(/,/g , ''))+' ksh <span>To</span> $'+CommaFormattedN(Number($('.balance_converted_to_dollar').attr('balance')))+',  <span> sell rate: </span>'+CommaFormattedN($("input#rate_dollar").val())+' ksh    </b>';
				}else if(Number($('.balance_converted_to_ksh').attr('balance'))){
					conv = '<b><b class="title_">Exchange:</b>  $'+CommaFormattedN($("#amount_money_dollar").val().replace(/,/g , ''))+'  <span>To</span> '+CommaFormattedN(Number($('.balance_converted_to_ksh').attr('balance')))+' ksh, <span>  puy rate: </span>'+CommaFormattedN($("input#rate_ksh").val())+'ksh    </b>';
				}
	...
	if(trans_type == 'exchange'){
				 ...
				 if(dollar_x_amount != '0' && ksh_x_amount != '0'){ // both 
			 	 ...
			 	 msg_type = ' <b> <b class="title_">'+trans_type+':</b> '+value+'  </b>'+conv;			
		} 
...
function delete_(data){
	$('#warning').html('<form style="display:none;" id="delete_form"><input type="hidden" name="id" value="'+data.id+'">  <input type="hidden" name="table" value="'+data.table+'">  </form> '+'<img src="css/warning.png" alt="warning" style="border-radius:2.5em; width:80px; height:40px; margin-right: 4px; "/>'+data.msg).dialog({  show: "blind", hide: "explode", width: 'auto',  height:'auto',  modal: false, buttons:  {
							"Yes": function() {
							 ...
							 submit_form($('#warning form').attr('id'),'my_company_system/php_files/delete.php');
...
function chosen_customer(selected_option ){   // e.g: $(this).find('option:selected');
    ...
    //  uncheck radio & checkboxes
    	//$('form#transaction_form input[type="radio"], form#transaction_form input[type="checkbox"]').prop('checked', false);
