...
function company_exit($com_name){
              $com_name = trim(strtolower($com_name));
        $taken_companies = explode(',',trim(strtolower(file_get_contents('../../../taken_companies')))); // no charactors not allowed   name,name1,
        return in_array($com_name, $taken_companies);
}
...
function get_report_totals($full_date_DB,$customer_id){
                $customer_id = sanitize($customer_id);
                $full_date_DB = sanitize($full_date_DB);

    $cust_id = (trim($customer_id) !='')?" and customer_id='$customer_id ":'';
    $totals = array('total_deposit_ksh' =>  mysqli_result_(mysqli_query_("select sum(amount_ksh) from transactions where type='in' and date like'%$full_date_DB%' $cust_id and delete_status!='1' "), 0), 'total_debt_ksh' => mysqli_result_(mysqli_query_("select sum(amount_ksh) from transactions where type='out' and date like'%$full_date_DB%' $cust_id and delete_status!='1' "), 0),
      'total_deposit_dollar' =>  mysqli_result_(mysqli_query_("select sum(amount_dollar) from transactions where type='in' and date like'%$full_date_DB%' $cust_id  and delete_status!='1' "), 0),'total_debt_dollar' => mysqli_result_(mysqli_query_("select sum(amount_dollar) from transactions where type='out' and date like'%$full_date_DB%'  $cust_id  and delete_status!='1' "), 0));
     
    $totals['cash_balance'] = $totals['total_deposit_ksh'] -  $totals['total_debt_ksh'];
    $totals['dollar_balance'] = $totals['total_deposit_dollar'] - $totals['total_debt_dollar'];

    $totals['debt_color'] = array('dollar' =>  ((preg_match('/-/', $totals['dollar_balance'] ))?'debt_color':(($totals['dollar_balance']!= '0')?'in_color':'') ),'ksh' =>  ((preg_match('/-/',$totals['cash_balance']))?'debt_color':(($totals['cash_balance']!= '0')?'in_color':'') ));
