...
$('div#wrapper').fadeOut();
 ...
 $.post('dynamic_backup.php', function () {
             $('#changing').dialog('close'); 
             // auto open the exported file 
         $('div#wrapper').fadeIn();
// respond feedback
$('#feedback').attr('title','updated').html('<img src="success.gif" style="border-radius:6em"> successful ').dialog({ position:'top',buttons:{'Okay': function (){					
...
// end of respond feedback
  ...
  currentDateDefault =  '';
 	function get_date(){
		$.post('get_Date.php', function(realDate){		
                  $('.date_manual').val(realDate);		
                     $('#name_exp').val(realDate);				  
		 });
	}
...
// change delete  password
	function changeDeletePass(){
       
	   $('#change_delete_pass input').val('');




	$('#change_delete_pass').dialog({ position:'top',buttons:{'change':function (){
 
	var c_pass      =  $('#password1_').val();



	var new_pass   =   $('#new_password_').val();



	var con_pass   =   $('#con_password_').val();
	...
	$('#error3333').html('password do not match!').fadeIn();
		...
		$.post('changeDeletePass.php',{   c_pass:c_pass ,new_pass:new_pass }, function (change_std_p) {	   
					...
					if(change_std_p == 2){	
						...
						$('#changing').dialog('close');  
	                 $('#change_delete_pass').dialog('close');


 
						$('#feedback_3').html('<img src="success.gif" style="border-radius:6em"> your Delete password has been Changed!').dialog({ position:'top',buttons:{'Okay': function (){
					...
					$('#error3333').html(change_std_p).fadeIn();
...
function exporter(query,title,colms,filename,two_tabs){
...
if(!query){
 return false;
    ...
    // start loading 
	 $('#changing').dialog({ position:'top',buttons:{}, closeOnEscape: true, draggable: false, resizable: false, modal: true, show:'fadeIn' });
 $.post('exporter.php',{query:query,title:title, filename:filename, colms:colms, two_tabs:two_tabs}, function (respond) {
             $('#changing').dialog('close'); 
             // auto open the exported file 


       
	 window.location.href = respond; // console.log(respond);
...
// edit customer update customer
 ...
 $('#description2').val('');
					...
					$('#edit_div').dialog({ position:'top',buttons:{'Okay': function (){
                            ...
                            var date_e		=  $('#date_customer_e').val();


						

                    if($.trim(date_e) == ''){
									$('#error2').html('please enter date !!').fadeIn();			
	 ...
	 $.post('edit.php',{date_e:date_e,id_or_passport_edit:id_or_passport_edit, description:description,id_edit:id_edit, edit_ful_name:edit_ful_name, edit_cash_in:edit_cash_in, edit_cash_out:edit_cash_out, edit_dol_in:edit_dol_in, edit_dol_out:edit_dol_out, cash_in:cash_in, cash_out:cash_out, dol_in:dol_in, dol_out:dol_out, edit_number:edit_number }, function (edited) {
...
$('document').ready( function () {
    $('.date_manual').datepicker({dateFormat:"dd/M/yy"});
...
// show home bage when i logged in 
 ...
 blace_side = 1;
	 ...
	 $.post('side_bar.php',{blace_side:blace_side}, function (people_blance) {
			...
			if(!$('div#search_resalt').is(':visible')){
					...
					$.post('history.php',{ id1: id1}, function (history) {
						 ...
						 if($.trim(dellPassword) == ''){
							 $('#worning_to_delete_exp').append('<p style="color:red;">please enter Delete Password !!<p>').fadeIn();
											...
											$.post('delete_histry.php',{dellPassword:dellPassword,delete_histryRow:delete_histryRow, id_card:id_card}, function (deletedhis) {
																													 ...
																													 $('#changing').dialog('close');  
																															...
																															$('#worning_to_delete_exp').append(deletedhis);
																															     ...
																															     $('#changing').dialog('close');  
																															      ...
																															      $('#Deleted').html('<img src="success.gif" style="border-radius:6em">transaction was Deleted').dialog({ position:'top',buttons:{'Okay': function (){ $(this).dialog('close');  }}, closeOnEscape: true, draggable: false, resizable: false,  modal: true, show:'fadeIn' });	
																										                    load_oppen_daily();
																														   } 
...
oppen_daily_custom_table();
								...
								$('#castomer_details a[edit="edit"]').click(function () {       
                                       ...
                                       var date_edit =  $(this).attr('date_edit');
										...
										$('#show_blance_cashx').text(blancex);
                                         ...
                                         $('#dollRate_').val($(this).attr('dollarrate'));
 ...
 calculationFunction();
							...
							$('#edit_exp_div').dialog({ position:'top',buttons:{'Okay': function (){
											   ...
											   var cashRate_ =  $('#cashRate_').val(); 			 			
												var dollRate_ =  $('#dollRate_').val(); 						
									 
					

											  var date_openday   =  $('#date_openday_e').val(); 						
											...
											if ($.trim(date_openday) == ''){

                                                   $('#error_exp').html('please enter date ').fadeIn();	
                                             }else{
													...
													$.post('edit_daily.php',{date_openday:date_openday,cashRate_:cashRate_,dollRate_:dollRate_,id_edit_x:id_edit_x, namex1: namex1, cash_inx1:cash_inx1, cash_outx1:cash_outx1, dol_inx1:dol_inx1, dol_outx1:dol_outx1 }, function (editedxpn) {
				...
				$('#worning_to_delete_exp').html('are sure you want to Delete '+date_del+'<p><label>  Password: </label> <input type="password" class="input" id="dellPassword" /> </p>').dialog({ position:'top',buttons:{'YES': function (){
					...
					$.post('delete_exp.php',{date_del:date_del,dellPassword:dellPassword}, function (deleted_exp) {
							...
							if (deleted_exp == 1){
								...
								$('#error_del').html('<p style="color: red;">Error Deleting  <strong>'+date_del+'</strong> !</p>').dialog({ position:'top',buttons:{'Okay': function (){
									...
									$('#worning_to_delete_exp').append(deleted_exp).fadeIn();
...
$('#debt_custmers').click( function () {
 ...
 // credits Customers tab
$('#credit_custmers').click( function () {
	...
	$('#totals').hide();
     ...
     $('#search').val('credit%'); search() // search all customers
	 $('#home_bage').show();
});
...
$('#new_exp_div').dialog({ position:'top',buttons:{'okay':function (){
		 ...
		 var date_openday   =  $('#date_openday_a').val(); 						
			 ...
			 if ($.trim(date_openday) == ''){
                          $('#error_exp2').html('please enter date ').fadeIn();	
              }else{
											...
											$.post('add_daily.php',{date_openday:date_openday,cashRate:cashRate,dollRate:dollRate,namex2: namex2, cash_inx2:cash_inx2, cash_outx2:cash_outx2, dol_inx2:dol_inx2, dol_outx2:dol_outx2 }, function (addedaily) {
...
$('#add_div').dialog({ position:'top',buttons:{'Add': function (){
            ...
            var date_a		=  $('#date_customer_a').val();


						

                    if($.trim(date_a) == ''){
									$('#error').html('please enter date !!').fadeIn();			
								...
								$.post('add.php',{date_a:date_a,id_or_passport:id_or_passport, desctiption:desctiption, new_ful_name: new_ful_name, new_cash_in:new_cash_in, new_cash_out:new_cash_out, new_doll_in:new_doll_in, new_doll_out:new_doll_out, new_number:new_number }, function (added) {
