	     ...
	     while($aRow = mysqli_fetch_assoc_($rResult))
			{
				...
				if(preg_match('/date/', $other_query)){
			  	...
			  	$trans_color = ( ($aRow['type'] == 'exchange' )?'exchange_color':((preg_match('/in:/',strtolower( str_replace(" ",'',$aRow['type_msg']))))?'in_color':'debt_color'));
				 $row[] = "<div class='$trans_color' style='border-bottom:none !important;'>".str_replace('<span', '<span class="span_" ', str_replace('title_', '',$aRow['type_msg']))."</div>"; 
