...
number_format(mysqli_result_(mysqli_query_("select sum(commission) from payments where ".(($date_to !='')?" sent_date BETWEEN '$full_date_DB' AND  '$date_to'  ":"sent_date like'$full_date_DB'")." and delete_status!='1' "),0),2)."  </b>  </p>
   </td>
  </tr>
...
<h3 class='title_'>(<strong>".number_format(mysqli_result_(mysqli_query_("select count(id) from payments where ".(($date_to !='')?" sent_date BETWEEN '$full_date_DB' AND  '$date_to'  ":"sent_date like'$full_date_DB'")." and delete_status!='1' "),0))."</strong>)  sent money for (<i>$full_date</i>) </h3> 
 

 <table  class='table dataTable'  other_query=\"$other_query_\"   table_file='php_files/pages/server-side-tables/all_payments.php' ><thead> <tr>  <th> </th>   </tr>  </thead>
<tbody>
 </tbody></table>


<h3 class='title_'>(<strong>".number_format(mysqli_result_(mysqli_query_("select count(id) from payments where ".(($date_to !='')?" status='paid' and sent_date BETWEEN '$full_date_DB' AND  '$date_to'  ":"  status='paid' and sent_date like'$full_date_DB'")." and delete_status!='1' "),0))."</strong>)  payments for (<i>$full_date</i>) </h3> 
 

 <table  class='table dataTable'  other_query=\"$other_query_ and  status='paid' \"   table_file='php_files/pages/server-side-tables/all_payments.php' ><thead> <tr>  <th> </th>   </tr>  </thead>
<tbody>
 </tbody></table>
 </div>
 ";
