				...
				$month = date("m/Y",$month);
						...
						$query_select = "SELECT `description`,`id_card` ,`id`,`full_name`, `cash_in`, `cash_out`, `blance` , `doller_in`,`doller_out`,`doller_blance`,`number`, `date` FROM `history` WHERE `id_card` = $id_2d and months='$month' order by date,id ASC"; 
							...
							if(@$query_run = mysql_query($query_select)){
														...
														$table_data .= '<tr><td>'.$full_name.'   <a href="#"  onclick="$(this).parent().click();" class="button ">view</a> </td><td>'.$cash_in.' </td><td>'.$cash_out.'</td><td>'.$blance.'</td> <td>$'.$doller_in.'</td> <td>$'.$doller_out.'</td> <td>$'.$doller_blance.'</td>  <td>'.$description.'</td><td>'.$date.' </td><td>  <a href="#" id="delete_histry" id_card="'.$id_card.'" delet_hist="'.$id.'"class="button delete"> delete </a></td></tr>'; 
														}
