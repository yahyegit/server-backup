...
die();
	...
	$query_select = 'SELECT `id`,dollarRate, `name`, `cash_in`, `cash_out`, `cash_in`- `cash_out` as `blance` , `dolla_in`,`dolla_out`,`dolla_in` - `dolla_out` as `dolla_blance`,`date` FROM `oppen_day`  WHERE `month`="'.getLastMonth('SELECT DISTINCT `month` FROM `oppen_day` WHERE 1','month').'" ORDER BY `id` DESC'; 
	
		if(@$query_run = mysql_query($query_select)){
		$table_data = ' <table class="table"   > <thead> <tr> <th>Title </th><th>Open Cash</th> <th> Open Dollar</th>  <th> Date </th>    <th> Dollar Rate </th>   <th> Action  </th>   </tr> </thead> <tbody>';
				...
				$table_data .= '<tr><td class="day" date="'.$date.'">'.$name.'  <a href="#"  onclick="$(this).parent().click();" class="button ">view</a>  </td><td>'.$cash_in.' </td><td>$'.$dolla_in.'</td><td>'.$date.'</td>  <td>'.$dollarRate.' </td><td><pre><a href="#"  id="'.$id.'" name="'.$name.'" cash_in="'.$sql_row['cash_in'].'" cash_out="'.$sql_row['cash_out'].'" blance="'.$sql_row['blance'].'" dolla_in="'.$sql_row['dolla_in'].'" dolla_out="'.$sql_row['dolla_out'].'" dollarRate="'.$sql_row['dollarRate'].'"  dolla_blance="'.$sql_row['dolla_blance'].'"  date_edit="'.$sql_row['date'].'"  edit="edit"  class="button ">edit</a> <a href="#"  id="'.$id.'"   date="'.$date.'"  delete="delet" class="button delete">Delete</a> </pre></td></tr>'; 
				}
