 ...
 $month_name		=   trim(mysql_real_escape_string(htmlentities($_POST['month_name'])));
	...
	if($month_name == 'ALL'){
		...
		$query_select = 'SELECT cashRate,`id`,dollarRate, `id`, `name`, `cash_in`, `cash_out`, `cash_in`- `cash_out` as `blance` , `dolla_in`,`dolla_out`,`dolla_in` - `dolla_out` as `dolla_blance`,`date` FROM `oppen_day`  ORDER BY `date`'; 
			...
			$table_data = ' <table class="table"   > <thead> <tr> <th>Title </th><th>Cash In </th>  <th> Dollar In </th> <th> Date </th>   <th> Cash Rate </th>  <th> Dollar Rate </th>    <th> Edit/Delete </th> </tr> </thead> <tbody>';
				...
				$dollarRate = 		number_format($sql_row['dollarRate']);
					...
					$table_data .= '<tr><td class="day" date="'.$date.'">'.$name.'</td><td>'.$cash_in.' </td>  <td>$'.$dolla_in.'</td> <td>'.$date.'</td> <td>'.$cashRate.'</td><td>'.$dollarRate.' </td>     <td><pre><a href="#"  id="'.$id.'" name="'.$name.'" cash_in="'.$sql_row['cash_in'].'" cash_out="'.$sql_row['cash_out'].'" blance="'.$sql_row['blance'].'" dolla_in="'.$sql_row['dolla_in'].'" dolla_out="'.$sql_row['dolla_out'].'" dolla_blance="'.$sql_row['dolla_blance'].'"   edit="edit"  class="button">edit</a> <a href="#"  id="'.$id.'"   date="'.$date.'"  delete="delet" class="button delete">Delete</a> </pre></td></tr>'; 
					}
