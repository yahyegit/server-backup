...
// get workers
 ...
 function  get_trucks($tt){
			 ...
			 $type_arry = Array();
				...
				$type_arry['profit'] = '<b style="color:'.(substr($profitCalc,1,strlen($profitCalc)) == '-')?'red':'green'.';">'.number_format($profitCalc,2).'</b>';
	     ...
	     if($data['status'] == 'add'){
			if($tableName == 'trucks'){
				...
				$truck_no = register($truck_no,$driverName,$driverLicenseNo,$mobile);  
						...
						mysql_query("INSERT INTO `trucks`(`id`, `truck_no`, `unit_price`,`quantity`,`driverName`, `driverLicenseNo`, `source`, `distination`, `cost`, `date`, `description`, `delete_status`,`mobile`) VALUES ('','$truck_no','$unit_price','$quantity','$driverName','$driverLicenseNo','$source','$distination','$cost','$date','$description','0','$mobile')");
				 if(!empty($excost)){ 
			 ...
			 if($tableName == 'trucks'){
					mysql_query("UPDATE `trucks` SET `unit_price`='$unit_price', `quantity`='$quantity',  `truck_no`='$truck_no',`driverName`='$driverName',`driverLicenseNo`='$driverLicenseNo',`source`='$source',`distination`='$distination',`cost`=$cost,`date`='$date',`description`='$description' WHERE `id`=$id");
				 }else if($tableName == 'expense'){
