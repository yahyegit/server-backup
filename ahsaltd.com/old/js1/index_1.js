...
function print(currentDataTable,currentTotals){
 ...
 function getWorkers(tabsNames_json,path){
	     ...
	     var section = '<div id="workers_tabs_div">';  // secction 
		 ...
		 var i = 0;
			 ...
			 cond = (work_type == 'All')?condFunction(projecName != '',projecs_query,''):' '+condFunction(projecName != '',projecs_query+' AND ','')+" `work-type`='"+work_type+"' ";
			    ...
			    section += '<h3>'+work_type+'<span class="badge" >('+totalsArray[2]+')</span> </h3>';	
				section += '<div id="w_tabs-'+i+'"> <table table_query="'+cond+'" table_class_name="workers_table'+unique_randon+'"  class="workers_tbl workers_table'+unique_randon+' display" cellspacing="0" width="100%" ></table>'; //  tab with total
...
//		  var complete_tabs = '<h3 class="h3">'+condFunction(projecName == '',add_btn,path2)+'</h3>  <div class="tabs_mulitple_all workers_tabs_div" style="clear:both;"><ul> '+type_tabs+'</ul>'+type_divs+'</div>';
 ...
 function getTrash(tabsNames_json,path){
	    ...
	    section = '<div id="trashAccordion" class="trash_tabs_div"><h3>Suppliers<span class="badge" >('+tabsNames_json.countSup+')</span> </h3>';	
			    section += '<h3>Products<span class="badge" >('+tabsNames_json.countProducts+')</span></h3>';	
				section += '<h3>Others<span class="badge" >('+tabsNames_json.countOthers+')</span></h3>';	
				section += '<h3>Workers<span class="badge" >('+tabsNames_json.countWorkers+')</span></h3>';	
				section += '<h3>Payments<span class="badge" >('+tabsNames_json.countPayments+')</span></h3>';	
				
			   section += '<div id="s_trash_tab"> <table class="suppleirs_trash_table display" cellspacing="0" width="100%" ></table> </div>'; //  tab with total
			   section += '<div id="pro_trash_tab"> <table class="products_trash_table display" cellspacing="0" width="100%" ></table> </div>'; //  tab with total
			   section += '<div id="others_trash_tab"> <table class="others_trash_table display" cellspacing="0" width="100%" ></table> </div>'; //  tab with total
			   section += '<div id="w_trash_tab"> <table class="workers_trash_table display" cellspacing="0" width="100%" ></table> </div> '; //  tab with total
               section += '<div id="p_trash_tab"> <table class="payments_trash_table display" cellspacing="0" width="100%" ></table> </div></div>'; //  tab with total
...
function getProducts(tabsNames_json,path){
           ...
           // creating tabs from objects
     var section = '<div id="products_tabs_div">';  // secction 
		 ...
		 var i = 0;
			 ...
			 $.each(tabsNames_json.product_types, function(productType, totalsArray){ 
			    ...
			    section += '<h3>'+productType+'<span class="badge" >('+totalsArray[2]+')</span></h3>';	
				section += '<div id="tabs-'+i+'"> <table  table_query="'+cond+'" table_class_name="products_table'+unique_randon+'"  class="products_tbl products_table'+unique_randon+' display" cellspacing="0" width="100%" ></table> ';
		 ...
		 // var complete_tabs = '<h3 class="h3"> '+path2+'</h3>  <div class="tabs_mulitple_all supplier_tabs_div" style="clear:both;"> '+section+'</div>';
...
function export_fun(projecName){
 ...
 // remove other btn from the printer
...
function template_creator(type,tabsNames_json,div_id){
	...
	if(jQuery.trim(type) == 'trash'){
          ...
          $('#'+jQuery.trim(div_id)+' #trashAccordion').accordion();
	...
	data_table_loader(' `total_balance`!=0 ','suppliers','supplier-account-id','supplier-account-id,supplier-name,mobile,total_total-Cost,total_paid,total_balance,created_date','debs_suppleirs_table');  // all suppliers table
           ...
           // creating tabs from objects
	     var section = '<div id="suppliers_history_accordion">';  // secction 
	   ...
	   $('#suppliers-history #suppliers_history_accordion').accordion();
	   $('#suppliers-history #suppliers_history_accordion').find('div').css('height','auto');
		     // end creating tabs
       ...
       was_visible = condFunction($('#'+jQuery.trim(div_id)).is(':visible'),1,0);
		...
		$('#'+jQuery.trim(div_id)+' #workers_tabs_div').accordion();
		$('#'+jQuery.trim(div_id)+' #workers_tabs_div').find('div').css('height','auto');
	 ...
	 // end creating tabs
	  ...
	  // applying dataTables
		 $('#'+jQuery.trim(div_id)+' .workers_tbl').each(function(){  
           ...
           // creating tabs from objects
	     var projects_section = '<div id="tabs_mulitple_all">';  // secction 
       ...
       $.each(tabsNames_json.projects, function(projecName,l){ 
                ...
                currentPath = '';
				floors_section = '<div id="tabs_mulitple_all">';  // secction 
					 ...
					 $.each(l, function(floorNo,count){ 
							   ...
							   if(floorNo != 'allCounts'){
                                path = projecName+'/'+condFunction(jQuery.trim(floorNo) == 'All','',floorNo);
								 ...
								 currentPath =  '<h3>'+path+'</h3> ';
								 
										  floor_content  = '<div id="tabs_mulitple_all">';  //  floor_content 
								  
											 floor_content += '<h3> Workers <span class="badge" >('+ww.work_types.All[2]+')</span> </h3><div id="flc-w-tabs-'+i+'_'+c+'"> '+getWorkers(ww,path)+'</div>'; // get workers
											 floor_content += '<h3> Products <span class="badge" >('+pp.product_types.All[2]+')</span> </h3>  <div id="flc-p-tabs-'+i+'_'+c+'"> '+getProducts(pp,path)+'</div>'; // get products
											 floor_content += ' <h3>Others <span class="badge" >('+oo.allCounts+')</span> </h3><div id="flc-o-tabs-'+i+'_'+c+'"> '+getOthers(oo,path)+'</div>'; // get Others
										   floor_content += '</div>';
							
				 
                                    floors_section += '<h3>'+floorNo+' </h3> <div id="fl_tabs-'+i+'_'+c+'"> '+floor_content+' </div>';  
							 
						       }
						 c++;	   
					   });
				projects_section += ' <h3>'+projecName+'<span class="badge" >('+l.allCounts+')</span> </h3><div>'+floors_section+'</div></div>';
...
//                  complete_floor_tabs = '<h3 style="border-bottom: 2px solid blue; width: 403px;">'+'<b style="color:blue;">'+f+'</b>'+path.slice(1)+' <button onclick=\'export_fun("'+projecName+'")\' class=" ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" style="/* float: left; */ font-size: 0.7em;width: 148px;height: 30px;font-weight: bold;border: 2px solid blue;" title="" role="button" aria-disabled="false"><span class="ui-button-icon-primary ui-icon ui-icon-print" style="zoom: 1.4;"></span><span class="ui-button-text"> Print Reports </span></button></h3> <div class="tabs_mulitple_all floors_tabs_div_'+i+'" style="clear:both;"><ul> '+floorsTabs+'</ul>'+floors_divs+'</div>';
	...
	//	complete_projects_tabs = ' <div class="projecs_tabs_main" style="clear:both;"><ul> '+projecs_tabs+'</ul>'+projecs_divs+'</div>';
	 ...
	 //complete_projects_tabs
 		...
 		$('#'+jQuery.trim(div_id)).fadeOut().html(progressBar+projects_section);
	//	$('#'+jQuery.trim(div_id)+' div.projecs_tabs_main').tabs();
		$('#'+jQuery.trim(div_id)+' #tabs_mulitple_all').accordion();
		$('#'+jQuery.trim(div_id)+' #tabs_mulitple_all ').find('div').css('height','auto').accordion();
		if(was_visible){
			$('#'+jQuery.trim(div_id)).fadeIn();	
		}
