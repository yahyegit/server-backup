 ...
 //$('table.'+jQuery.trim(tableClassName)).closest("div").find('.fg-toolbar').find('select').append('<option value="all">all</option>');
  ...
  function data_table_loader(other_query,tableName,primaryKey,collms,tableClassName){
	  ...
	  if(jQuery.trim(tableName) == 'trucks'){ // suppliers  main tabs  all and debt
	 		    ...
	 		    // aplly datable now 
	         apply_dataTable(other_query,tableName,primaryKey,collms,tableClassName,'<thead><tr><th>Truck No</th><th>Current Driver</th><th>last date</th><th class="actions_collm_data">Actions</th></tr></thead>');
...
function template_creator(type,tabsNames_json,div_id){
    ...
    $.each(tabsNames_json, function(type, totalsArray){ 
	    ...
	    type_tabs =  '<div class="trash_tabs" style="clear:both;"><ul>  <li><a href="#tabs-1"> Trucks </span></a> </li><li><a href="#tabs-2"> expenses </a></li> </ul> ';
		    type_tabs += '<div id="tabs-1"> <table class="truckTrashTable display" cellspacing="0" width="100%" ></table>  </div>'; // all supplier table with total
		    type_tabs += '<div id="tabs-2"> <table class="expenseTrashTable display" cellspacing="0" width="100%" ></table>   </div>'; // expense history  totals
	    type_tabs += '</div>';
	     ...
	     apply_dataTable('trash','trucks','id','id,truck_no,driverName,driverLicenseNo,source,distination,cost,date,description','truckTrashTable','<thead><tr><th>Truck No</th><th>Driver Name</th><th>driver License No</th><th>From</th><th>To</th><th>Cost</th><th>Date</th><th>Description</th><th class="actions_collm_data">Actions</th></tr></thead>');
       ...
       clear_main_divs(div_id);
		...
		$('li a[href="#'+jQuery.trim(div_id)+'"]').html('<span class="badge" >('+tabsNames_json.allCounts+') Trucks </span>');
          ...
          data_table_loader('unique_tracks','trucks','id','id,truck_no,driverName,date','all_trucks_table');  // all suppliers table

	
	  }else if(jQuery.trim(type) == 'drivers'){ // suppliers main tabs debs and all with e.g Debs(x)
       clear_main_divs(div_id);
	        add_btn = '<h3 class="h3"> <a class=" ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" style="float: right; font-size: 0.7em; float: left;" title=" " role="button" aria-disabled="false" onclick="add($(this).parents().parents().find(\'table.dataTable:first th:first\'),$(this).parents().parents().find(\'table.dataTable:last th:first\'),getAddArgs(\'trucks\',\'\',\'\',\'add\'),\'\');"><span class="ui-button-icon-primary ui-icon ui-icon-circle-plus"></span><span class="ui-button-text">Add </span></a></h3>';
	        type_tabs = add_btn+  ' <table class="all_drivers_table display" cellspacing="0" width="100%" ></table> '; 
		...
		$('li a[href="#'+jQuery.trim(div_id)+'"]').html('<span class="badge" >('+tabsNames_json.allCounts+') drivers </span>');
				...
				apply_dataTable('unique_drivers','trucks','id','id,driverName,mobile,driverLicenseNo,date','all_drivers_table','<thead><tr><th>Driver Name</th><th>Mobile</th><th>driver License No</th><th>last date</th><th class="actions_collm_data">Actions</th></tr></thead>');
	  ...
	  // console.log(tabsNames_json);
	    ...
	    title = (jQuery.trim(type) == 'truckHistry')?' Truck '+tabsNames_json.id+'':'Driver history  history > '+tabsNames_json.driverName+ ', License no: ' +tabsNames_json.licenseNo+' '+tell;
		
	   
	   
	    type_tabs = add_btn+'<div class="truckHistry_tabs" style="clear:both;"><ul>  <li><a href="#tabs-1"> '+truck_+'</span></a> </li><li><a href="#tabs-2"> <span class="badge" >('+tabsNames_json.countExp+')</span> expenses </a></li> </ul> ';
	   ...
	   otherQueryEx = (jQuery.trim(type) == 'truckHistry')?'`name` like "%, Truck No ('+$.trim(tabsNames_json.id)+')%"':'`name` like ", Driver license No ('+$.trim(tabsNames_json.id)+')%"';
	     ...
	     apply_dataTable(otherQuery,'trucks','id','id,truck_no,driverName,driverLicenseNo,source,distination,cost,date,description','truckHistryTable','<thead><tr><th>Truck No</th><th>Driver Name</th><th>driver License No</th><th>From</th><th>To</th><th>Cost</th><th>Date</th><th>Description</th><th class="actions_collm_data">Actions</th></tr></thead>');
 ...
 var title = '';
  ...
  if(data_array['tableName'] == 'trucks'){
	  ...
	  $('div#add .trucks').parent().closest('li').css({'display' : '' });
		...
		confirm_thead = '<th>Truck No</th><th>From</th><th>To</th><th>Cost</th><th>Description</th><th>Driver Name</th><th>driver License No</th><th>mobile</th><th>driver Expense</th><th>Date</th>';
  ...
  totals_thead  = '<th> Total Cost </th> ';
     ...
     if(data_array['status'] == 'editTruck'){
	   ...
	   $('div#add .truck_no,.driverName').css({'display' : '' });
	   $('div#add .truck_no,.driverName').parent().closest('li').css({'display' : '' });
		   ...
		   $('div#add .driverName,.driverLicenseNo,.mobile').parent().closest('li').css({'display' : '' });
			 ...
			 // Default data while editing 
				...
				$('div#add .eecost').hide().parent().closest('li').hide(); // hide from editing 
				  ...
				  $('div#add .cost').val(data_json.cost);
				   ...
				   if(data_array['tableName'] == 'trucks'){
							...
							lengthData = data['name'].length;
							  ...
							  for (x = 0; x < lengthData; x++) { 
									 if(data_array['tableName'] == 'trucks'){
											confirmation +=   '<tr> <td>'+data['truck_no'][x]+'</td> <td>'+data['source'][x]+'</td><td>'+data['distination'][x]+'</td><td>'+CommaFormattedN(data['cost'][x])+'</td><td>'+data['description'][x]+'</td><td>'+data['driverName'][x]+'</td><td>'+data['driverLicenseNo'][x]+'</td><td>'+data['mobile'][x]+'</td><td>'+data['ecost'][x]+'</td><td>'+data['date'][x]+'</td></tr>';
									  }else if(data_array['tableName'] == 'expense'){
											confirmation +=   '<tr> <td>'+data['name'][x]+'</td> <td>'+CommaFormattedN(data['quantity'][x])+'</td><td>'+data['type'][x]+'</td><td>'+CommaFormattedN(data['excost'][x])+'</td><td>'+data['edate'][x]+'</td><td>'+data['edescription'][x]+'</td></tr>';
									   }
						  ...
						  $("div#confirmation table:first").dataTable({
							 ...
							 $('div [aria-describedby="confirmation"] div:first span').text(conf_title); 
									       ...
									       loading_func(); // SHOW LOADING animation dialog    
										 	...
										 	$.post('php_f/add.php', { id:data_json.id,tableName:data_array['tableName'],status:data_array['status'],name:data['name'],type:data['type'],edate:data['edate'],excost:data['excost'],edescription:data['edescription'],quantity:data['quantity'],truck_no:data['truck_no'],cost:data['cost'],date:data['date'],description:data['description'],driverLicenseNo:data['driverLicenseNo'],driverName:data['driverName'],ecost:data['ecost'],mobile:data['mobile'],source:data['source'],distination:data['distination'] }, function(feedback_add) {					
