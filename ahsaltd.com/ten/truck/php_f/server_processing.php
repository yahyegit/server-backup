	...
	$output = array(
		...
		"aaData" => array()
	     ...
	     while($aRow = mysql_fetch_array( $rResult ))
			{
				...
				if($sTable == 'expense'){
					...
					$row_edit['truck_no'] =  str_replace(')','',str_replace(' Truck No (','',$exName[1]));    
				...
				if( $sTable == 'trucks'){
					...
					$filterdTotals['excost'] =  $filterdTotals['excost'] + $rowExp;
         ...
         if($filteredTotalsStatus){
			...
			$fiterd_title_excost = (preg_match('/-/',$_GET['sSearch']))?"Total Cost for (".$_GET['sSearch'].')':'total  Filterd Cost';  
			   ...
			   if( $sTable == 'trucks'){ // trucks
				...
				$output['aaData'][] = Array("<div class='weaklyTotals'><table class='table'><tbody><tr><th>$fiterd_title_price</th> <td  class='td_not_remove_dt'>".number_format($filterdTotals['cost'],2)."</td> <th>$fiterd_title_excost</th> <td  class='td_not_remove_dt'>".number_format($filterdTotals['excost'],2)."</td> <th>$fiterd_title_profit</th> <td  class='td_not_remove_dt'>".$profitFormat."</td>  </tbody></table></div>");
