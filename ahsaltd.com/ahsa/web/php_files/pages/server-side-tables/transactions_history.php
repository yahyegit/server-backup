	     ...
	     while($aRow = mysqli_fetch_assoc_($rResult))
			{
				...
				if(!preg_match('/customer_id/', $other_query)){
					...
					$row[] = "<button  title=\"click to see all transactions for $customer_name_  \" class='change ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary' onclick=\"r_page('',{customer_id:{$aRow['customer_id']}},'pages/other_pages/customer_acount_page.php')\" role='button' aria-disabled='false' style='font-size: 11px;'><span class='ui-button-icon-primary ui-icon  '></span><span class='ui-button-text'>$customer_name_</span></button><br>".'<i style="margin-left: 7px;font-weight:normal; border-bottom:1px solid #fafbfc; "  >'.mysqli_result_(mysqli_query_("select mobile from customers where customer_id={$aRow['customer_id']}"), 0).'</i>';
					 
				}   



				$row[] = '<span >ksh'.number_format($aRow["cash_in"],2).'</span>';
				$row[] = '<span   >ksh'.number_format($aRow["cash_out"],2).'</span>';
				$row[] = '<span class="'.toggle_debt_color($aRow["cash_balance"]).'" >ksh'.number_format($aRow["cash_balance"],2).'  </span>';
				$row[] = '<span >$'.number_format($aRow["dollar_in"],2).'</span>';
				$row[] = '<span   >$'.number_format($aRow["dollar_out"],2).'</span>';
				$row[] = '<span class="'.toggle_debt_color($aRow["dollar_balance"]).'" >$'.number_format($aRow["dollar_balance"],2).'</span>';

			  	 

					if(preg_match('/customer_id/', $other_query)){
				 	 $row[] = '<a href="#" title="view transactions for '.$customer_name_.' on ('.date('d/M/Y',strtotime($aRow['date'])).')" class="change ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary"
