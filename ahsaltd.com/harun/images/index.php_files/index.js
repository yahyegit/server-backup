...
function data_table_creator(json_data,carDetails = ''){
		...
		if(json_data.status == 'cars'){   
			//  (all cars, available cars)
			 available_cars_badget = ($.trim(json_data.available_cars_count) != '0')?'<b class="badge">'+json_data.available_cars_count+'</b>':'';
			 $('#view_container').html(add_car_btn+'<div id="cars_tabs"  style="margin-top: 10px;"> <ul><li><a href="#all_cars">all cars</a></li> <li><a href="#available_cars">Availble Cars  '+available_cars_badget+' </a></li></ul>    <div id="available_cars"> <div class="totals"></div><table id="available_cars_table" class="table" width="100%" where_query="'+json_data.where_query_available+'"  table_type="cars" table="'+json_data.table_available+'" primary_key="'+json_data.primary_key_available+'" colmns="'+json_data.colmns_available+'"><thead> <tr> '+json_data.thead_available+'  </tr></thead></table> </div>      <div id="all_cars"> <div class="totals"></div><table id="all_cars_table" class="table" width="100%" where_query="'+json_data.where_query_all_cars+'" table="'+json_data.table_all_cars+'" primary_key="'+json_data.primary_key_all_cars+'" table_type="cars"   colmns="'+json_data.colmns_all_cars+'"><thead> <tr> '+json_data.thead_all_cars+'  </tr></thead></table> </div>       </div>');
			 ...
			 $('#view_container table').hide();
			 			// get car details
			$.post('php_f/get_car_details.php',{car_id:carDetails},function(feedback){
				if($.trim(json_data) == 'login'){location.reload();}
				$('div.car_details').html(feedback).fadeIn();
			});	
