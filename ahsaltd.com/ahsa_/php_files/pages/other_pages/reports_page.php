...
".((ctype_digit($totals['open']['id']))?"
        <p class=\"underline \"> <i><span>Open cash :</span></i> <b class=\"\">".number_format($totals['open']['amount_ksh'],2)." ksh,  dollar rate: ".number_format($totals['open']['dollar_rate'],2)."</b>    

".((!empty($date_to))?'':"<button class=\"change ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary\" onclick=\"
           request_template('',{dollar_rate:{$totals['open']['dollar_rate']},id:{$totals['open']['id']},amount_ksh:{$totals['open']['amount_ksh']},date:'$full_date_DB'},'pages/forms/open_day_form.php');\" role=\"button\" aria-disabled=\"false\" style=\"font-size: 11px;\"><span class=\"ui-button-icon-primary ui-icon ui-icon-pencil\"></span><span class=\"ui-button-text\">Edit </span></button>")."

          </p> 
 
<p class=\"underline \"> <i><span> Your Current Total Cash Balance is :</span></i> <b class=\"".toggle_debt_color($totals['open']['amount_ksh_balance'])."\">".number_format($totals['open']['amount_ksh_balance'],2)." ksh </b>     </p>
