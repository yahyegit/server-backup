 ...
 function get_limit_history($data){ // y,m,d
     ...
     $current_send_limit  =  number_format(mysqli_result_(mysqli_query_("select current_send_limit from users where id='$id' and delete_status!='1' "),0),2);

      
$data = "<div class='reports_page'>
 
  <p class=\"title_\" style='margin:auto auto;color:#7c11a2 !important;text-align: center;margin-bottom: 13px;margin-top: 20px;'> Limit history for <b>$username</b> </p>

 
 <p style='margin:auto auto;width: 493px;/* border: 2px solid; */ ' ><label class=\"show_ float_label\" > current payment limit</label> <b class='debt_color'>".'$'."$Remaining<b> </p>

  <p  style='margin:auto auto;width: 493px;/* border: 2px solid; */' ><label class=\"show_ float_label\" > current sending limit</label> <b class='debt_color'>".'$'."$current_send_limit<b> </p>
...
\" class=\"ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary\" onclick=\"r_page('',{user_id:$id},'pages/forms/add_limit_form.php');\"  role=\"button\" aria-disabled=\"false\"><span class=\"ui-button-icon-primary ui-icon ui-icon-plus\"></span><span class=\"ui-button-text\"> Add limit to <b>$username</b></span></button> <br>
 ...
 <table  class='table dataTable'  other_query=\"user_id='$id' \"   table_file='php_files/pages/server-side-tables/limit_history.php' ><thead> <tr> 
							   <th> sending limit </th> <th> payment limit </th>   <th> username </th>  <th> description </th>   <th> Date </th> <th> action </th> 
							 </tr> </thead>
<tbody>
 </tbody></table>
