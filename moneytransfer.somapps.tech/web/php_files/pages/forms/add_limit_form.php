...
global $current_user;
   ...
   $data = mysqli_fetch_assoc_(mysqli_query_("SELECT `id`, `amount`, `user_id`, `description`,`send_amount`, `date` FROM `limit_history` where id='$id' ")); 
...
<table class='table' style='    margin-left: 0px;width:auto !important; box-shadow: none !important;'>
 ...
 <tr>
   ...
   <td > <div style='display:;margin-left: 12px;'><label class=''>Payment Amount</label><br> 
 
   $<input type='text'   onkeyup=\"remaining_calc_();\"   format_comma='true'   name='amount' placeholder=\" Payment Amount \" value='{$data['amount']}'     > 
  </div></td>

   <td > <div style='display:;margin-left: 12px;'><label class='show_'>current payment limit</label><br> 
