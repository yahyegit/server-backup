      ...
      $data['amount'] = str_replace(',','',$data['amount']);
           ...
           $data = clean_security($data);
            $password = (!empty($data['password']))?",password='".enc_password($data['password'])."'":'';
             ...
             $remain_limit = mysqli_result_(mysqli_query_("select current_send_limit from users where id=$current_user "),0);
             ...
             if(preg_match('/-/',$remain_limit-$data['amount'])){
                  exit('invalid <b>amount</b><br> your current sending limit is: <b>$'.number_format($remain_limit).'<b> ');
                } 
                  ...
                  update_limit($current_user,$prev_amount,'+s');
                   update_limit($current_user,$data['amount'],'-s');
