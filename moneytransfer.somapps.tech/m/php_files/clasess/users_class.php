            ...
            $password = (!empty($data['password']))?",password='".enc_password($data['password'])."'":'';
             if (!empty($data['id'])) {
                     if (!user_exists($data['username'],$data['id'])){
                      ...
                      // update 
                       mysqli_query_("UPDATE `users` set full_name='{$data['full_name']}',   username='{$data['username']}' $password where id={$data['id']}"); 
        return 'ok';  
                    }else{
                 ...
                 if (!user_exists($data['username'],'')){
                        mysqli_query_("INSERT INTO `users`(`current_lang`,`full_name`, `username`, `password`,`type`,status) VALUES('en','{$data['full_name']}','{$data['username']}','".enc_password($data['password'])."','staff','1')");
                      }else{
                      return "sorry <strong> {$data['username']} </strong> is already exists .";
                    }
