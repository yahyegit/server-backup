	     ...
	     while($aRow = mysqli_fetch_assoc_($rResult))
			{
			 ...
			 if (preg_match('/subject/', $other_query)  ){
					  ...
					  if(!mysqli_result_(mysqli_query_("select name from students where student_id='{$aRow['student_id']}' and delete_status!='1' and status='1'"),0)){
					  			continue;
					  }


				     $row[] = mysqli_result_(mysqli_query_("select name from students where student_id='{$aRow['student_id']}' and delete_status!='1' and status='1'"),0);
