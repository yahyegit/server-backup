...
function get_transction_form($data){
   ...
   $data['balance'] =  mysqli_result_(mysqli_query_("select sum(balance) from invoices where delete_status!='1' and balance!='0' and student_id='$customer_id'"),0);
 return "
...
<h4 class='title_'> <strong> {$data['name']}</strong> is making payment. </h4>
...
\"><label class='show_'>Balance</label><br> 
     <input type='text' name='balance' value='".number_format($data['balance'],2)."'  bl='{$data['balance']}' disabled='disabed'   > </div>
    </td>
</tr> 
...
\"><label class=''>One Time Discount</label><br> 
   <input type='number' onchange='display_balance_calc();'  onkeyup='display_balance_calc();' name='discount' placeholder=\"One time Discount   \" value=''  style='display:;' > </div>
  </td>
</tr>
