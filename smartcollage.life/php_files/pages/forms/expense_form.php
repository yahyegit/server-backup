...
function get_expense_form($data){
 ...
 return "
  ...
  $('input[name=\"type\"]').autocomplete({
      source: ".get_autoComplate("select distinct type from expenses where delete_status!='1'",'type')."
    });
...
\"><label class=''>Quantity</label><br> 
   <input type='number'  name='quantity' placeholder=\" quantity is optional \" value='{$data['quantity']}'  style='display:;' > </div>
  </td>
</tr>
...
\"><label class=''>type</label><br> 
   <input type='text'   $disa  error_empty_msg='required'    required name='type' placeholder=\"enter Type \" value='$type'  style='display:;' > </div>
  </td>
</tr>
