...
// get workers
 function  get_history($s,$type,$id){
			...
			$queryex = (trim($type) == 'truckHistry')?"`name` like '%, Truck No ($id)'":"`name` like '%, Driver license No ($id)'";
				...
				$type_arry['eCost'] =  number_format(mysql_result(mysql_query("SELECT sum(`cost`) FROM `expense` WHERE $query_ AND `delete_status` !='1' "),0),2);
                $type_arry['eQuantity']=  number_format(mysql_result(mysql_query("SELECT sum(`quantity`) FROM `expense` WHERE $query_ AND `delete_status` !='1'"),0));
			    ...
			    $type_arry["countExp"] = number_format(mysql_result(mysql_query("SELECT count(DISTINCT `id`) FROM `expense`  WHERE $query_ AND `delete_status` !='1' "),0));
 ...
 function  get_trucks($tt){
			 ...
			 $type_arry = Array();
				...
				$type_arry["allCounts"] = number_format(mysql_result(mysql_query("SELECT count(DISTINCT `$tt`) FROM trucks  WHERE `delete_status` !='1'  "),0));
...
// getAvailable Tags for AutoComplate
 ...
 function  getAvailableTags(){
					$jsonData = Array();				
						...
						$jsonData[''] = getData("SELECT DISTINCT `distination` FROM `trucks` WHERE `delete_status` !='1'",'distination');
...
function load($data_arry){   // type and id 
 ...
 $data_arry['type'] = sanitize($data_arry['type']);
  ...
  // start 	 
	...
	}else if($data_arry['type'] == 'drivers'){  //drivers

		   return get_trucks('driverLicenseNo');
...
//   print_r($data);
		  ...
		  for ($i=0; $i<$dataLenth; $i++) { 
		    ...
		    $truck_no = sanitize($data['truck_no'][$i]);
				...
				$expenseName = "$expenseName, $exdriverLicenseNo, $extruck_no";
				
				 
			// detect run queries
	     if($data['status'] == 'add'){
			if($tableName == 'trucks'){
						mysql_query("INSERT INTO `trucks`(`id`, `truck_no`, `driverName`, `driverLicenseNo`, `source`, `distination`, `cost`, `date`, `description`, `delete_status`,`mobile`) VALUES ('','$truck_no','$driverName','$driverLicenseNo','$source','$distination','$cost','$date','$description','0','$mobile')");
				 if(!empty($ecost)){ // extra expense
						mysql_query("INSERT INTO `expense`(`id`, `name`, `quantity`, `type`, `cost`, `description`, `date`, `delete_status`) VALUES ('','$driverName ($driverLicenseNo)','1','extra','$ecost','','$date','0')");
					}
			 
			 }else if($tableName == 'expense'){
                mysql_query("INSERT INTO `expense`(`id`, `name`, `quantity`, `type`, `cost`, `description`, `date`, `delete_status`) VALUES ('','$expenseName','$quantity','$type','$excost','$eDescription','$edate','0')");
            }
			
		 }else if($data['status'] == 'edit'){
			
			 if($tableName == 'trucks'){
					mysql_query("UPDATE `trucks` SET `truck_no`='$truck_no',`driverName`='$driverName',`driverLicenseNo`='$driverLicenseNo',`source`='$source',`distination`='$distination',`cost`=$cost,`date`='$date',`description`='$description' WHERE `id`=$id");
				 }else if($tableName == 'expense'){
					mysql_query("UPDATE `expense` SET `name`='$expenseName',`quantity`='$quantity',`type`='$type',`cost`='$excost',`description`='$eDescription',`date`='$edate' WHERE id='$id' ");
            }
		 }else if($data['status'] == 'editDriver'){
			        mysql_query("UPDATE `trucks` SET `driverName`='$driverName',`driverLicenseNo`='$driverLicenseNo',mobile='$mobile' WHERE `driverLicenseNo`='$id'");
				    mysql_query("UPDATE `expense` SET `name`='`name`($driverLicenseNo)' WHERE name like '%$id%' ");
		 }else if($data['status'] == 'editTruck'){
			        mysql_query("UPDATE `trucks` SET `truck_no`='$truck_no',`driverName`='$driverName' WHERE `truck_no`='$id'");
				    mysql_query("UPDATE `expense` SET `name`='`name`($truck_no)' WHERE name like '%$id%' ");
		 }
