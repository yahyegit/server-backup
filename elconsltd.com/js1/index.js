				...
				"iDisplayLength": 25,			       
                   ...
                   "fnCreatedRow": function( nRow, aData, iDataIndex ) {
                              ...
                              $('td:first div.weaklyTotals', nRow).each(function (){
							      ...
							      // add class 'actions_collm_data' if in action collms td 
							     $('td:last', nRow).css('display','').addClass(condFunction(table_thead.match(/action/g),'actions_collm_data',''));
...
function template_creator(type,tabsNames_json,div_id){
	...
	data_table_loader(' `total_balance`!=0 ','suppliers','supplier-account-id','supplier-account-id,supplier-name,mobile,total_total-Cost,total_paid,total_balance,created_date','debs_suppleirs_table');  // all suppliers table
	   ...
	   //$('#suppliers-history div.supplier_tabs_div').tabs();
		  ...
		  $.each(tabsNames_json.product_types, function(productType, totalsArray){ 
					...
					data_table_loader(cond,'suppliers-history','id','id,product-name,product-type,quantity,single-cost,total-Cost,paid,balance,date,project-name,floorNo','suppleir_table'+i); 
				return false;
					  }
				i++;
				});
	 ...
	 //complete_projects_tabs
		...
		//$('#'+jQuery.trim(div_id)+' div.tabs_mulitple_projects').tabs();
				$('#'+jQuery.trim(div_id)+' div.tabs_mulitple_projects_').tabs();
		if(was_visible){
			$('#'+jQuery.trim(div_id)).fadeIn();	
		}
		...
		// done
		 ...
		 $('#'+jQuery.trim(div_id)+' .products_tbl').each(function(){  
			   var cond =  $(this).attr('table_query');
			   var uniqueClassName = $(this).attr('table_class_name');
			           data_table_loader(cond,'suppliers-history','id','id,supplier-name,mobile,product-name,product-type,quantity,single-cost,total-Cost,paid,balance,date,project-name,floorNo',uniqueClassName); 
				});  
 ...
 function autocompleteCach(parent_elem){
	       ...
	       if($.trim(parent_elem) == ''){
			    $(".dataTables_filter input,.workType,.name,.mobile,.identity,.productName,.productType,.project_name,.floorNo").each(function(){
            ...
            // $(".dataTables_filter input").autocomplete("destroy");
			   	// autocomplete dataTable
   				 $(".dataTables_filter input").autocomplete({
					  source: jQuery.unique($.merge($.merge(autoComplateData['project_names'],autoComplateData['floorNos'],autoComplateData['product_types'],autoComplateData['product_Names']),$.merge(autoComplateData['workTypes'],autoComplateData['names'], autoComplateData['mobiles'],autoComplateData['identitys'])))
					  });
											...
											"Agree": function() {
											 ...
											 if(jQuery.trim(payment_name) != jQuery.trim(data['name'][0])){
											    ...
											    //console.log(data['date']);
											          // agreed  
									                  loading_func(); // SHOW LOADING animation dialog  
										 	          $.post('php_f/add.php', {date:data['date'],paymentDate:paymentDate,description_payment:description_payment,payment_amount:payment_amount,tableName:data_array['tableName'],account_id:account_id,name:data['name'],mobile:data['mobile'], id_or_passport:data['id_or_passport'], work_type:data['work-type'], number_of_workers:data['number-of-workers'],w_single_cost:data['w_single_cost'],cost:data['cost'], product_name:data['product-name'],product_type:data['product-type'], quantity:data['quantity'],  single_cost:data['single-cost'], total_cost:data['total-Cost'], project_name:data['project-name'], floorNo:data['floorNo'],description:data['description']}, function(feedback_add) {					
				        ...
				        var data = new Array();
                            ...
                            if(data_json.tableName == 'suppliers'){
                              ...
                              // end of data grupping an validating 
									                  ...
									                  loading_func(); // SHOW LOADING animation dialog  
										 	          $.post('php_f/edit.php', {id:data_json.id,date:data['date'],tableName:data_json.tableName,name:data['name'],mobile:data['mobile'],w_single_cost:data['w_single_cost'], id_or_passport:data['id_or_passport'], work_type:data['work-type'], number_of_workers:data['number-of-workers'],cost:data['cost'], product_name:data['product-name'],product_type:data['product-type'], project_name:data['project-name'], floorNo:data['floorNo'],description:data['description'],}, function(feedback) {					
