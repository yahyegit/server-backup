...
// reports date select
function chosen_cust_report_date(){
    customer_id = $("input#date_from").attr('customer_id');
	from_date = $("input#date_from").val();
	to_date = $("input#date_to").val();
 
	 r_page('',{customer_id:customer_id,date_to:to_date,date_from:from_date},'pages/other_pages/customer_acount_page.php');
      //  $(".tran_totals").fadeIn();
	}
...
function delete_(data){
	$('#warning').html('<img src="css/warning.png" alt="warning" style="border-radius:2.5em; width:80px; height:40px; margin-right: 4px; "/>'+data.msg+'<br><br><div class="dell_form"></div>');
	    $.post('php_files/pages/forms/login_form_delete.php',{data:data}, function (feadback) {
 								$('#warning .dell_form').html(feadback);
 						     $('#warning .dell_form .form').tee_form(); 
          });
    $('#warning').dialog({position: { my: "center bottom", at: "center center", of: window }, show: "blind", hide: "explode", width: 'auto',  height:'auto',  modal: true});
