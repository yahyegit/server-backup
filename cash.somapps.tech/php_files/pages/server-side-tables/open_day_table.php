	     ...
	     while($aRow = mysqli_fetch_assoc_($rResult))
			{
			    ...
			    $row[] =  $aRow["title"];
		 	  	 ...
		 	  	 $row[] = '<a href="#" title="view reports for ('.date('d/M/Y',strtotime($aRow['date'])).')" class="change ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary"
				 	onclick="get_template.reports({date_from:\''.$aRow["date"].'\'},\'pages/other_pages/reports_page.php\');" role="button" aria-disabled="false" style="font-size: 11px;"><span class="ui-button-icon-primary ui-icon ui-icon-clock"></span><span class="ui-button-text">'.date('D, d/M/Y',strtotime($aRow['date'])).'</span></a>';
				 	// open cash 
                ...
                // open cash balance 
			 	...
			 	$row[] = '<span>'.number_format($aRow["dollar_rate"],2).' ksh</span>';
  				...
  				// actions 
				$row[] = "<div  class='hide_for_print'> <button class=\"change ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary\" onclick=\"
