...
function make_transaction($data){
 ...
 $amount_data['currency'][$i] = trim($amount_data['currency'][$i]);
 $amount_data['conv_to_currency '][$i] = trim($amount_data['conv_to_currency'][$i]);
...
\"> <div >  prev  balance:'."<span class='".toggle_debt_color($prev_balance[$amount_data["conv_to_currency"][$i]])."'>".strtoupper($amount_data["conv_to_currency"][$i]).number_format($prev_balance[$amount_data["conv_to_currency"][$i]],2)."</span></div>

  <div >  new balance: <span class='".toggle_debt_color($new_balance[$amount_data["conv_to_currency"][$i]])."'>".strtoupper($amount_data["conv_to_currency"][$i]).number_format($new_balance[trim($amount_data["conv_to_currency"][$i])],2).'</span></div>
    </div>   
</div>
';
...
\"> <div >  prev  balance:'."<span class='".toggle_debt_color($prev_balance[$amount_data["currency"][$i]])."'>".strtoupper($amount_data["currency"][$i]).number_format($prev_balance[$amount_data["currency"][$i]],2)."</span></div>
  ...
  <div >  new balance: <span class='".toggle_debt_color($new_balance[$amount_data["currency"][$i]])."'>".strtoupper($amount_data["currency"][$i]).number_format($new_balance[trim($amount_data["currency"][$i])],2).'</span></div>
    </div>   
</div>
';
