...
function chosen_item(el){
    elm = el.closest('tr');
  	if (el.val() == 'choose..'){return false;}

			if (el.val() == 'Add new'){
				elm.find('input[name="item_name"]').fadeIn().val('');
				elm.find('input[name="price"]').val(0);
				elm.find('input[name="remainings"]').attr('rem',0);
			}else{
				elm.find('input[name="item_name"]').hide().val(el.val());

	elm.find('input[name="price"]').val(el.attr('price'));
    elm.find('input[name="remainings"]').attr('rem',el.attr('remainings'));
		}
		
		elm.find('input[name="remainings"]').val(CommaFormattedN(clean(elm.find('input[name="remainings"]').attr('rem'))+clean(elm.find('input[name="quantity"]').val())));


}

function update_bl_item(el){
 	el.find('input[name="remainings"]').val(CommaFormattedN(clean(el.find('input[name="remainings"]').attr('rem'))+clean(el.find('input[name="quantity"]').val())));
...
if (bll.toString().includes('-')) {
		 $('input[name="balance"]').attr('style','border:2px solid red !important;');

	error_func('invalid balance check paid amount or discount !');

 }else{
 	 $('input[name="balance"]').removeAttr('style');//('border','1px solid red !important;');
...
// reports date select
function chosen_date(file){
 	year = $("select.year").val();
	month = $("select.month").val();
if ($.trim(month) == 'full year') {
 		 date = year;
	...
	request_template('',{date:date},'pages/other_pages/'+$.trim(file)); 
...
function clean(value){  
 ...
 return 0;}   
  ...
  $('input[name=sender_name], input[name=pay_to_name]').autocomplete({
        source:  {"0":"hey, (0700000000000'), ","2":"test name, (88888888888'), ","3":"to name 1, (0710000000'), ","9":"haah, (06111111111111'), ","10":"sender name 1, (06100000000'), ","11":"sender name 1, (06100000000'), ID\/Passport No:111111111111111"}   
       }); 

function chosen_report_date(file){

	from_date = $("input#date_from").val();
	to_date = $("input#date_to").val();
 
	 get_template.reports({id:$("input#date_from").attr('user_id'),date_to:to_date,date_from:from_date},'pages/other_pages/'+$.trim(file));
...
function success_fun(msg){
	...
	// display seccess massage
  	$('#success').html('  <img src="css/check.svg" alt="success" style="border-radius:2em; width:60px; height:40px; margin-right: 4px; "/>'+msg).fadeIn();
...
function error_func(msg){
	...
	if($('.form').is(':visible')){
			...
			$('div#error').css('width','auto').css('margin','');
                              ...
                              if($('#tabs_login').is(':visible')){
                                  $('#error').css({"margin-right":'0px',"width":'34%'});
                              }else{
                                  $('#error').css({"margin-right":'auto',"width":'50%'});
                              }
  	$('div#error').html('<img src="css/error.svg" alt="error" style="border-radius:2em; width:60px; height:40px; margin-right: 4px;"/>'+msg+"").fadeIn(); 
...
function delete_(data){
	$('#warning').css('color','red !important;').html('<form class="form" style="display:none;" id="delete_form"><input type="hidden" name="id" value="'+data.id+'">  <input type="hidden" name="table" value="'+data.table+'">  </form> '+'<img src="css/warning.svg" alt="warning" style="border-radius:2.5em; width:80px; height:40px; margin-right: 4px; "/>'+data.msg).dialog({  show: "blind", hide: "explode", width: 'auto',  height:'auto',  modal: true, buttons:  {
							"Yes": function() {
...
function toggle_disable_(data){
	$('#warning').css('color','#9F6000 !important;').html('<form  class="form"  style="display:none;" id="delete_form"><input type="hidden" name="id" value="'+data.id+'">  <input type="hidden" name="table" value="'+data.table+'">  </form> '+'<img src="css/error.svg" alt="error" style="border-radius:2em; width:60px; height:40px; margin-right: 4px;"/>'+data.msg).dialog({  show: "blind", hide: "explode", width: 'auto',  height:'auto',  modal: false, buttons:  {
							"Yes": function() {
 ...
 function Loguut(){
 	 $('#warning').html('<img src="css/warning.svg" alt="warning" style="border-radius:2.5em; width:80px; height:40px; margin-right: 4px; "/>Are you Sure you want to LogOut ?').dialog({  show: "blind", hide: "explode", width: 'auto',  height:'auto',  modal: true, buttons:  {
							"Yes": function() {
