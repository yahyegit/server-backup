...
$last_order = mysqli_result_(mysqli_query_("SELECT order_number FROM `invoices` order by id desc limit 1 "), 0);
...
<tr class='first_row'>
  ...
  <td> <label class=' float_label  '>Quantity <i>(quantity)</i> </label><br> 
    <input type='number' name='quantity' placeholder='Quantity' onkeyup=\"realTimeBl(); \" onchange=\"realTimeBl(); \" error_empty_msg='required'  required  > </td>
...
<table class='table' style='     margin-left: 0px;width:auto !important; box-shadow: none !important;'>
 ...
 <td class='c_info' style='display:none;'> <label class=' float_label'>address <i>(optional)</i></label>  </br>
    <input type='text' name='address' placeholder=\"address (optional)  \" value='' > </td>
 </tr>
...
<td> <label class=''>date</label><br> 
    ...
    <td> <label class=''>Invoice No</label><br> 
    <input type='number' name='order_number' placeholder='Invoice No' value='$last_order' error_empty_msg='required'  > </div>
   </td>
