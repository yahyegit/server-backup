...
//  <th> item </th> <th> quantity </th>   <th> cost </th>  <th> Date </th> <th> description </th>  <th> action </th> 
        ...
        // <thea 
	     while($aRow = mysqli_fetch_assoc_($rResult)){
          ...
          $row = array();
          $cost =  $ccc.number_format($aRow['cost']/$aRow['quantity'],2)."<span class='gray' style='font-style:italic;'> total: ". $ccc.number_format($aRow['cost'],2).'</span>'; 
				$description = (!empty($aRow['description']))?"<p>".$aRow['description']."</p>":''; 	  
          			$row[] =  "<p> <label class='float_label show_'>item </label> ".$aRow['item_name']."</p> <p><label class='float_label show_'>tirada <i>(quantity)</i> </label> ".number_format($aRow['quantity'])."</p> <p><label class='float_label show_'>cost </label> $cost   </p> <p><label class='float_label show_'>date </label> ".date('Y-M-d',strtotime($aRow['date']))."</p> $description  <p> <button class='  ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary' onclick=\"delete_({table:'$sTable',id:{$aRow['id']},msg:'Deleting received item ?'}); \" role='button' aria-disabled='false' style='  display:inline;'><span class='ui-button-icon-primary ui-icon ui-icon-trash'></span><span class='ui-button-text'>Delete   </span></button>   
