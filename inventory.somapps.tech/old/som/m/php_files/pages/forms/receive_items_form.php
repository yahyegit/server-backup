...
<table class='table'>
 ...
 <td class='csn'><label>item name</label><br>	<input type='text' error_empty_msg='required' style='display:none;' placeholder='new item name' name='item_name'  required> <select onchange=\"chosen_item($(this).find('option:selected'))\"> ".get_items($id)." </select>  </td>

</tr>

   <tr>
 
 <td><label class=''>tirada <i>(quantity)</i>  </label><br>	<input   type='number' error_empty_msg='required' onkeyup=\"update_bl_item()\" placeholder='tirada' onchange=\"update_bl_item()\"  name='quantity' value='' required>      </td>  </t><tr> </td>   <td><label class='show_' >remainings</label><br> <input type='text'  placeholder='remainings' name='remainings' value='' disabled='disabled'  >      </td>  </tr>

   <tr>

<td><label>Qarashka kusogaday xabadiiba <i>(cost)</i></label><br>  $ccc<input type='number' style=\"
    width: 324px;
\" onkeyup=\"update_bl_item()\" error_empty_msg='required'  placeholder='Qarashka kusogaday xabadiiba (cost) ' name='cost' value=''  required>  </td> </t><tr> <td><label class=''>Inta iibinaysid <i>(price)</i></label><br>  $ccc<input type='number' error_empty_msg='required'  placeholder='Inta iibinaysid (price)' name='price' value=''  required>  </td></tr>
