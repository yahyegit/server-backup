...
function empty_check(error_empty_msg,val,elm){
            	...
            	if($.trim(error_empty_msg) !='' && val == '' || val == '..' || val == '0' || val == '0.00' || val == 'choose..'){
            		 	...
            		 	if(elm.find('option')){
            		 			elm.closest('tr').find('.chzn-container:first').addClass('error_border');
            		 	...
            		 	if(elm.find('option')){
            		 		 console.log('select> ');
            		 				...
            		 				console.log( elm.closest('tr').find('.chzn-container').removeClass('error_border').html());
...
el = this.find('.form_footer_btns .ui-button:last');
...
fruits = ['register_form_l','login_form_l'];
 if (!fruits.includes(form_id)) {
       this.dialog({  show: "blind", hide: "explode", width: 'auto',  height:'auto',  modal: true});
}
...
this.prepend(' <div id="error" title="Error" style="display:none;"></div>');
        ...
        /*    $('#'+form_id+' *').blur(function(){
          ...
          // submit on enter
           $('#'+form_id+' *[name]').each(function(){
    		...
    		//this.find("input[type='checkbox'],input[type='radio']").show();
