...
function chosen_item(el){
  	...
  	if (el.val() == 'choose..'){return false;}
			...
			if (el.val() == 'Add new'){
 	$('input[name="item_name"]').fadeIn().val('');
$('input[name="price"]').val(0);
    $('input[name="remainings"]').attr('rem',0);
			}else{
				 	$('input[name="item_name"]').hide().val(el.val());

	$('input[name="price"]').val(el.attr('price'));
    $('input[name="remainings"]').attr('rem',el.attr('remainings'));
        }
   update_bl_item();
...
function update_bl_item(){
	$('input[name="remainings"]').val(CommaFormattedN(clean($('input[name="remainings"]').attr('rem'))+clean($('input[name="quantity"]').val())));
}
...
function toggle_disable_(data){
 ...
 function Loguut(){
 	 $('#warning').html(' <img src="css/warning.svgm" alt="warning" style="border-radius:2.5em; width:80px; height:40px; margin-right: 4px; "/>Are you Sure you want to LogOut ?').dialog({  show: "blind", hide: "explode", width: 'auto',  height:'auto',  modal: true, buttons:  {
							"Yes": function() {
