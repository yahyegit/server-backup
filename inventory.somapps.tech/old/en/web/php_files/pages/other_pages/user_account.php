...
\" > Reports for <b>{$user['username']}</b> from <input  class='title_' style=\"
    font-weight:bold; \"   type='date' id='date_from'  user_id='{$user['id']}' value=\"$full_date_DB\"  onchange=\"chosen_report_date('user_account.php');\" >   To 

 <input      class='title_' style=\"
    font-weight:bold; \"      type='date' id='date_to'  value=\"$date_to\" style=\"display: ".(($date_to !='')?'':'none').";  \" onchange=\" if($.trim($('#date_from').val()) != ''){chosen_report_date('user_account.php');}\">  </p>
 ...
 <div id='items'>
		  ...
		  <p><label class='show_ float_label' > Total price : </label>$ccc".number_format(mysqli_result_(mysqli_query_("select sum(price) from invoices where user_id='{$user['id']}' and $date_qry and delete_status!='1' "),0),2)." </p>
