...
function register_supplier($data){
    ...
    if(!empty($id)){
      ...
      mysqli_query_("insert into suppliers (current_balance,mobile,s_name) values('".sanitize($data['balance'])."','".sanitize($data['mobile'])."','".sanitize($data['s_name'])."')");
...
function recieve_items($data){
  ...
  if(check_token($data['crf_code'],'check')){
        ...
        for ($i=0; $i <  count($data['items_data']['item_name']); $i++) { 
                  ...
                  $cost = sanitize($data['items_data']['cost'][$i])* $quantity;  
                  $data_i = array('price' =>$price ,'item_name' =>$item_name,'quantity' =>$quantity,'cost' =>$cost );
                      ...
                      $item_id = register_item($data_i);
                            ...
                            mysqli_query_("INSERT INTO `recieved_history`(`order_number`,`item_id`, `item_name`, `cost`, `quantity`, `s_id`,`date`) VALUES ('$order_number','$item_id','$item_name','$cost','$quantity','$s_id','$date') ");
        }
    ...
    if (!empty($data['amount'])){
          mysqli_query_("INSERT INTO s_payments (s_id,date,paid)VALUES('$s_id','$date','{$data['amount']}')");
         $debt = sanitize($data['balance'])-$data['amount'];
 
          mysqli_query_("update suppliers set current_balance=current_balance+$debt  where id=$s_id");  
