...
function submit_form(form_id,file_name){
	...
	data['courses_data'] = amount_data;
						c_p_oc = $('select#payment_amount_selected').find('option:selected');
				  			          	...
				  			          	if(login_ == 'login' || $.trim(feedback) == ''){
                     						window.location.reload();
		                    						...
		                    						loading_fun('stop',''); // finish the loading 
						 							if($.trim(feedback) == 'ok'){
						 								// success
														 if($('select#payment_amount_selected').length){
															window.open('tel:*799*'+c_p_oc.attr('account')+'*'+c_p_oc.attr('cost')+'\#');		 
