 ...
 (function( $ ) {
    ...
    $.fn.dataTable_loader = function() {
		      ...
		      other_query = encodeURIComponent(this.attr('other_query'));
		       other_query2 = encodeURIComponent(this.attr('other_query2'));

		      file = this.attr('table_file');   // customers.php
			  primary_key = encodeURIComponent(this.attr('primary_key'));
			  table = encodeURIComponent(this.attr('table'));
			  table_c = this;
			 this.DataTable({
                   "bProcessing": true,
				   "bServerSide": true,
...
console.log('tab: '+table_c.find('tr').length);
    if($(this).closest('table.dataTable:first tr').length <2 ){
    //		window.location.reload();
       ...
       $('.ui-icon-trash').closest('.ui-button').css('background','#ff000061');
				 	...
				 	if($('.detect_talble tr').length < 9){
	                 			// remove toolbar 
	                	$('.detect_talble').parent().find('div.fg-toolbar').remove(); 
 				        $('.detect_talble').removeClass('detect_talble ');
 				    }
