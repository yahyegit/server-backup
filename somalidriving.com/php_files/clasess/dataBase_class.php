               ...
               return 'login';   
                  ...
                  if(isset($type['username'])){  // authenticate now 
                           if(mysqli_result_(mysqli_query_("select count(username) from settings where password='".enc_password($type['password'])."' and username='".sanitize($type['username'])."' "),0) == '1'){
                               ...
                               $current_user = mysqli_result_(mysqli_query_("select id from settings where password='".enc_password($type['password'])."' and username='".sanitize($type['username'])."'  "),0);
                                $_SESSION['user_id'] = $current_user;

$location = 'http://somalidriving.com';//'https://' . $_SERVER['HTTP_HOST'];
