...
$stable_spe = 'upaid_class'; 
	$aColumns = explode(",",'student_subject');
	$sIndexColumn =  'id';  
	$other_query = " balance='0' and status='1' " ;
...
$exist = array();
 ...
 if(!in_array($aRow['student_subject'], $exist)){
 		$exist[] = $aRow['student_subject'];
		  	 ...
		  	 $row[] = "(".number_format(mysqli_result_(mysqli_query_("select count(id) from invoices where  student_subject='{$aRow['student_subject']}' and $other_query and delete_status!='1' and status='1' "), 0)).") paid students <button class='change primary_button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary' onclick=\"request_template('','{$aRow['student_subject']}','pages/other_pages/paid_single_subject.php');\" role='button' aria-disabled='false' style=' display:inline;'><span class='ui-button-icon-primary ui-icon ui-icon-'></span><span class='ui-button-text'>Show students </span></button> 
 
 ";

  	 $row[] = "<span style='color:#7c11a2'>$".number_format(mysqli_result_(mysqli_query_("select sum(balance) from invoices where  student_subject='{$aRow['student_subject']}' and $other_query and delete_status!='1' "), 0))."</span>";
 	                     $output['aaData'][] = $row;  
			  } 	        
			} 
