...
if(time() > strtotime('26-June-2020')){
  ...
  if(strtotime('17-June-2020') < time()){
            die("<p> tijaabadii way dhamaatay laxiriir WhatsApp ka  +25261-6311-168");
...
//echo enc_password(sanitize($type['password'])); die();
        ...
        if(!empty($type['company_name'])){
           $connection->select_db('college_'.sanitize(str_replace("'", '', preg_replace('/[^a-zA-Z0-9\']/', '',$type['company_name']))));
        } 
        // auth
        if($type == 'logout'){
               session_destroy();
                  ...
                  return true;   
                   ...
                   if(isset($type['username'])){  // authenticate now 
                            if(mysqli_result_(mysqli_query_("select count(username) from users where password='".enc_password($type['password'])."' and username='".sanitize($type['username'])."' and delete_status!='1' and status='1' "),0) == '1'){
                                 ...
                                 $_SESSION['last_action'] = time();
                                 $_SESSION['db_name'] =  sanitize('college_'.sanitize(str_replace("'", '', preg_replace('/[^a-zA-Z0-9\']/', '',$type['company_name'])))); 
...
$q = mysqli_query_("select subject from student_subjects where student_id='$id' and delete_status!='1' ");
 ...
 $no_of_sub = mysqli_result_(mysqli_query_("select count(subject) from student_subjects where student_id='$id' and delete_status!='1' "),0);
$origal = $no_of_sub;
 $bre = '';
 $bre_count = 1;
    ...
    while($aRow = mysqli_fetch_assoc_($q))
      {
           ...
           $g .= " $s <a href='#' onclick=\"request_template('',{student_subject:'{$aRow['subject']}'},'pages/other_pages/student_subjects.php');\" style=' display:inline !important; color:#7c11a2 !important; ' title='click to see students for {$aRow['subject']}'>{$aRow['subject']}</a>   
          $bre   ";
...
fix_invoices();
 ...
 function get_total_class_bl($subject){
      $total = 0;
      $qa = mysqli_query_("select balance from  invoices where student_subject='$subject' and status='1' and balance!='0' and delete_status!='1' ");
      while ($aRow = mysqli_fetch_assoc_($qa)) {
        # code...
            // skip disabled 
             if(!mysqli_result_(mysqli_query_("select name from students where student_id='{$aRow['student_id']}' and delete_status!='1' or status='1'"),0)){
                        continue;
                  }else{
                    $total =  $total+$aRow['balance'];
                  }
                  

      }
    ///  echo "select * from  invoices where student_subject='$subject' and status='1' and balance!='0' and delete_status!='1' ";
      return $total;
}
