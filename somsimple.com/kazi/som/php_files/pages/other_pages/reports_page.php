...
<p class=\"underline \"> <i><span>Open cash balance :</span></i> <b class=\"".toggle_debt_color($totals['open']['amount_ksh_balance'])."\">".number_format($totals['open']['amount_ksh_balance'],2)." ksh </b><span> and </span><b class=\"".toggle_debt_color($totals['open']['amount_dollar_balance'])."\">$".number_format($totals['open']['amount_dollar_balance'],2)." </b>     </p>
 ...
 <table class='table' style='
    ...
    margin-top: 20px;
    /* margin-left: 0px; */
'><thead><tr> <th class='title_2'>  Totalka xisaabadka <i>($full_date)</i> </th> </tr><tr></tr></thead>
<tbody>
  <tr> <td style='
    background: #fafbfc;
'>
...
<h3 class='title_'>(<strong>".number_format(mysqli_result_(mysqli_query_("select count(id) from transactions where ".(($date_to !='')?" date BETWEEN '$full_date_DB' AND  '$date_to' ":"date like'$full_date_DB'")." and delete_status!='1' "),0))."</strong>) xisaabood (<i>$full_date</i>) </h3> 
</div>
